...
describe('Role translations', () => {
  const allRoles: UserRole[] = ['user', 'mentor', 'admin'];
  ...
  it('should have matching role translation keys for en and ko', () => {
    ...
    // Should have exactly 3 role translations
    expect(enRoleKeys).toHaveLength(3);
  });

  it('should map UserRole values to translation keys', () => {
    // Verify the naming convention: role 'user' -> roleUser
