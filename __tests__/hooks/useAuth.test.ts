...
describe('UserRole type', () => {
  it('should include user role for no-permission users', () => {
    const validRoles: UserRole[] = ['user', 'mentor', 'admin', 'super_admin'];
    ...
    validRoles.forEach(role => {
      expect(['user', 'mentor', 'admin', 'super_admin']).toContain(role);
    });
  });

  it('should have user as the lowest permission level', () => {
    // Document the expected permission hierarchy
    const permissionHierarchy: UserRole[] = ['user', 'mentor', 'admin', 'super_admin'];

    expect(permissionHierarchy[0]).toBe('user');
    expect(permissionHierarchy[3]).toBe('super_admin');
  });
});
